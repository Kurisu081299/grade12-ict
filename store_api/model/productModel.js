...
productModel.postProduct = (data, callback) =>{
    dbConn.query("INSERT INTO products_tbl (name, description, price, stock, category) VALUES (?, ?, ?, ?, ?)", [data.name, data.description, data.price, data.stock, data.category], (error, result) => {
    if (error){
...
productModel.updateProduct = (product_id, data, callback) => {
    dbConn.query(
      "UPDATE products_tbl SET name=?, description=?, price=?, stock=?, category=? WHERE product_id=?",
  ...
  productModel.deleteProduct = (idsToDelete, callback) => {
    const query = "DELETE FROM products_tbl WHERE product_id = ?";
